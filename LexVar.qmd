...
Across many languages, child-directed speech (CDS) frequently includes "baby-talk" words that are unique, or at least highly specific, to this register [@bynon1968berber; @casagrande1948comanche; @ferguson1964baby; @han2023che; @mazuka2008japanese; @ota2018choo; @pye1986quiche; @sarvasy2025child]. Child-directed words are often variants of existing adult word forms, involving phonological and/or morphological changes [e.g., *doggy* for *dog*, @moore2024wordform] or replacement by onomatopoeic forms [e.g., *woof-woof* as an alternate label for *dog*: @laing2017perceptual; @ota2018choo], though some share little to no overlap in form or meaning [e.g., *bunny* for *rabbit*: @soderstrom2007beyond]. While such child-directed variants are typically replaced by adult forms later in development [@casey2022doggy], evidence from prior naturalistic corpus studies indicates that, in the first years of life, multiple labels for the same referent are used seemingly interchangeably in children’s input [@caseyIPcorpus; @moore2024wordform]. Here, we investigate how such label variation impacts children's word learning.

From a traditional one-to-one mapping perspective, label variation could be considered "noise" in the learning signal. Children learn to associate individual word forms (i.e., labels) with individual referents [e.g., @smith2008infants; @stevens2017pursuit; but see also @wojcik2022map]; thus, label variation complicates this mapping problem. Indeed, children often avoid adopting a second label for a referent they already have a name for [i.e., the mutual exclusivity bias, @markman1988children; @markman2003use; @merriman1989mutual; but see also @mervis1994two]. Using multiple variants to label the exact same referent also violates the pragmatic assumption that unique word forms must not share the exact same meaning [i.e., the principle of contrast: @clark1988logic]. Introducing multiple interchangeable labels also effectively decreases the frequency of the standard label [one of the best predictors of earlier word learning; @braginsky2019consistency], makes utterances less predictable [and thus potentially harder to learn new words from; e.g., @reuter2019predict], creates possible lexical competition [@swingley2007competition], and perhaps increases memory interference [@regier2005emergence], or generally increases memory demands for young learners [see @vlach2019learning for further discussion of memory constraints].

Yet, (American) English-hearing children both recognize [@potter2024frequent] and use [@casey2022doggy; @deak2001any] multiple labels for the same referent. Children's earliest learned words are associated with the greatest amount of label variation in their input [@caseyIPcorpus; @moore2024wordform]. Moreover, the presence of certain types of label variation in the input has been shown to correlate both with children's concurrent vocabulary size [@callanan2004multiple] and future vocabulary growth [@ota2018choo] as well as their earlier learning of individual words [@moore2024wordform]. Thus, young learners appear to successfully contend with---and potentially benefit from---label variation in their input.

One partial explanation for this counterintuitive pattern is that child-directed variants are more learnable than standard adult word forms. Reduplicated variants, which include repetition of full or partial syllables (e.g., *woof-woof*, *bow-wow*), are relatively common in CDS [though estimated rates vary substantially across languages, e.g., @han2023che; @mazuka2008japanese; @ota2018choo]. Ota and Skarabela [-@ota2016reduplicated] found that (Scottish) English-learning 17- to 19-month-olds learned reduplicated labels for novel objects better than non-reduplicated labels. Diminutivized variants, which include /i/ epenthesis in English (e.g., *doggy*), are also common in CDS [though again with cross-linguistic variation in rates, CITES]. Moore and Bergelson [-@moore2024wordform] found that diminutives were the most common type of label variation observed in a corpus of 6- to 17-month-old (American) English-learning infants. Critically, hearing a higher proportion of both reduplicated words and diminutivized words has been linked to faster vocabulary growth from 9 to 21 months for (Scottish) English learners [@ota2018choo]. Additionally, both of these types of label variation have been shown to facilitate word segmentation [@ota2018reduplication; @kempe2007diminutives], and in doing so, could support word learning [e.g., @yurovsky2012statistical].

What prior findings do not address is how the use of multiple variants in the *same* interaction could support children's word learning. We consider attention as a plausible mechanism by which label variation could facilitate---rather than hinder---word learning, even over short timescales. Just as CDS as a register is generally preferred by young listeners over adult-directed speech [ADS, @manybabies2020quantifying], certain features of CDS variants may heighten children's engagement across repeated labeling instances. 
...
* In line with the vast majority of prior word learning studies conducted to date, which overwhelmingly test children's recognition of one-to-one mappings, we hypothesized that children would demonstrate successful learning in the *consistent* condition. 
