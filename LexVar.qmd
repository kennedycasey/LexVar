...
# Introduction
Across many languages, speech addressed to infants and young children frequently includes vocabulary items unique or highly specific to this register [@bynon1968berber; @casagrande1948comanche; @ferguson1964baby; @han2023che; @mazuka2008japanese; @pye1986quiche; @sarvasy2025child]. These register-specific lexical variants, also commonly referred to as "baby-talk words" or "infant- or child-directed vocabulary"...

Multiple lexical variants have been shown to be used seemingly interchangeably in English child-directed speech [@caseyIPcorpus; @casey2022doggy; @moore2024wordform]. Earlier-learned words tend to be associated with more variation [@caseyIPcorpus; @moore2024wordform], suggesting that learners must contend with a relatively complicated linguistic landscape from early in development. How do multiple labels impact children’s word learning?

## Label variation complicates early word learning
The process of early word learning is traditionally framed as a one-to-one mapping problem—children learn to associate individual word forms (i.e., labels) with individual referents [@smith2008infant; but see also @wojcik2022map].
...
[^1]: Beta regression was chosen to model the proportional dependent variable [@cribari2010beta]. Specifically, we used a zero-one-inflated model since 0s and 1s are common values in our data [@liu2018review]. Note that this particular modeling approach requires a Bayesian framework to interpret. Importantly, this analytic approach allows us to quantify evidence in support of the null hypothesis, not simply against it. A frequentist beta regression using the `glmmTMB` package [@glmmTMB] roughly approximated the same trend but is limited in two key ways: (a) this model cannot accurately capture boundary values [see @liu2018review for relevant discussion] and (b) this model fails to converge with the maximal random effects structure. Most relevant, removing the random slope of condition to achieve convergence reduces our ability to detect within-subject effects of condition.
...
Although the interaction between condition and pupil synchrony was not statistically reliable, the direction of the effect in the typical variation condition suggested a possible trend toward increased accuracy with higher synchrony at both the sentence and word level.
