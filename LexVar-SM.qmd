...
m.age.comp <- anova(m.age, m.age.int) %>% tidy()

# maximal model does not converge --> singularity issues --> simplify
...
m.age.effects <- tidy(m.age, effects = "fixed", conf.int = TRUE)
m.window1.effects <- tidy(m.window1, effects = "fixed", conf.int = TRUE)
m.window2.effects <- tidy(m.window2, conf.int = TRUE)

emmeans(m.age, ~ condition, at = list(age_c = 0), type = "response")
emmeans(m.window1, ~ condition, at = list(age_c = 0), type = "response")
emmeans(m.window2, ~ condition, at = list(age_c = 0), type = "response")
```
